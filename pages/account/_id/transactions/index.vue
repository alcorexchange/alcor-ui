...
export default {
  ...
  computed: {
    ...mapState(['markets_obj']),
    ...mapState('market', ['base_token', 'quote_token', 'id']),
    loaded() {
      return this.markets_obj[0] && this.accountDeals.length
    },
    ...
    virtualTableData() {
      ...
      const data = this.accountDeals
        .map(deal => ({
          ...deal,
          id: deal._id,
          side: this.$route.params.id == deal.bidder ? 'buy' : 'sell',
  ...
  mounted() {
    this.getAccountDeals()
  },

  methods: {
    ...mapActions(['fetchAccountDeals']),
    async getAccountDeals() {
      this.accountDeals = await this.fetchAccountDeals(this.$route.params.id)
    },
    redirect(item) {
      if (this.isMobile)
