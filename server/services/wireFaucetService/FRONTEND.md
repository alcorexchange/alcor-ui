...
⚠️ **Но дёрнуть ручки из браузера пока нельзя.** `networkResolver` определяет чейн
**только по поддомену** — query-параметра для этого нет, — а записи
`wiretest.alcor.exchange` в Cloudflare не заведено. Из curl проверяется подменой
Host (`curl -H 'Host: wiretest.alcor.exchange' …`), для фронта нужна проксируемая
A-запись на `65.109.128.4`.
...
## Блокер: браузер не может ходить в ноду напрямую

Нода на `http://116.202.36.122:6666`, и туда фронт не достучится **по трём
...
Все три чинятся одним прокси-путём на nginx (`wax.alcor.exchange` → нода).
**Сейчас его нет, надо добавить.** Пять строк в `nginx/wire-testnet.conf`,
скажи — сделаю.

**С 29.08.2026 блокер стал уже.** Читать цепь через прокси больше не нужно —
это делает бек биржи (см. раздел выше), фронт берёт данные из его API. Прокси
остался нужен ровно для двух вещей, которые бек за юзера сделать не может:

| Что | Куда | Зачем прокси |
|---|---|---|
| `push_transaction` | `/v1/chain/push_transaction` | подпись юзера уходит в цепь только с фронта |
| `get_info` | `/v1/chain/get_info` | `chain_id` и TAPOS-заголовок для сборки транзакции |

Логин (`get_accounts_by_authorizers`) тоже идёт в ноду, но его можно завернуть
в бек, если не хочется открывать цепь наружу целиком.

Пока прокси нет, фронту доступны эндпоинт регистрации и (когда поднимут инстанс)
читающие ручки биржи. Подписать и отправить транзакцию — нельзя.
...
Пусто — значит новый юзер, шлём на `/register`. Ходить туда фронт сможет
только через прокси из раздела выше.
...
4. Поднять прокси до ноды — нужен, чтобы юзер мог подписать и отправить транзакцию.
5. Первая транзакция от лица юзера (`transfer`), чтобы убедиться в подписи.

Экраны с пулами и позициями от этого списка не зависят: они рисуются по API
биржи и упираются только в поддомен `wiretest.alcor.exchange` (см. первый
раздел). Их можно делать параллельно.
