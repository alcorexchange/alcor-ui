...
spot.get('/pairs', cacheSeconds(3, (req, res) => {
...
spot.get('/:market_id/charts', async (req, res) => {
  const network = req.app.get('network')
  const market_id = parseInt(req.params.market_id)
  if (isNaN(market_id)) return res.status(403).send('Invalid market id')
...
spot.get('/:market_id', cacheSeconds(60, (req, res) => {
...
spot.get('/:market_id/orderbook', async (req, res) => {
  const market_id = parseInt(req.params.market_id)
  if (isNaN(market_id)) return res.status(403).send('Invalid market id')
